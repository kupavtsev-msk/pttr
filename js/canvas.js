...
$(document).ready(function($) {
    ...
    $('form[name="append-text"]').submit(function() {
        //создаем див для того, что-бы измерить размеры
        let textarea = $(this).find('textarea'),
            text = textarea.val(),
            htmlText = text.replace(/\n/g, '<br>');
        let font = {
                family: textarea.css('font-family'),
                size: 200,
                align: 'left',
                lineHeight: 1,
                color: '#222222'
            },
            div = $('<div>'+htmlText+'</div>').css({
                fontSize: font.size,
            ...
            body = $('body');
        body.append(div);
        let width = div.width()+10,
            height= div.height(),
            canvas = $('<canvas width="'+width+'px" height="'+height+'px"></canvas>');//создаем канвас с нужными размерами
        div.remove();
        //пишем в канвас
        canvas.drawText({
        ...
        let png = canvas.getCanvasImage('png');
        canvas.remove();
        main_canvas.addImage(png);
        return false;
    });
    ...
    $(document).contextmenu({
        delegate: "#main-canvas",
        autoFocus: true,
        preventContextMenuForPopup: true,
        preventSelect: true,
        taphold: true,
        menu: [
            {title: "Download SVG", cmd: "get_svg"},
            {title: "Download PNG", cmd: "get_png"},
            {title: "Print", cmd: "print"}
        ],
        open: function() {
            main_canvas.canvas.click();
        },
        ...
        select: function(event, ui) {
            ...
            switch(ui.cmd){
                case "get_svg": {
                    let ctx = new C2S(main_canvas.canvas.width(), main_canvas.canvas.height()); //width, height of your desired svg file
                    //do your normal canvas stuff:
                    let base_image = new Image();
                    base_image.src = main_canvas.getPng();
                    ctx.drawImage(base_image, 0, 0, main_canvas.canvas.width(), main_canvas.canvas.height());
                    var s = new XMLSerializer().serializeToString(ctx.getSvg());
                    var encodedData = window.btoa(s);
                    console.log(encodedData);
                    let link = $('<a href="data:image/svg+xml;base64,'+encodedData+'" download="pattern.svg"></a>');
                    link[0].click();
                    //let svgContent = ctx.getSerializedSvg(true);
                } break;
                case "get_png": {
                    let link = $('<a href="'+main_canvas.getPng()+'" download="pattern.png"></a>');
                    link[0].click();
                } break;
                case "print": {
                    main_canvas.print_canvas();
                } break;
            }
        }
    });
...
class layers_group {//класс отвечающий за группы изображений
    constructor(canvas, src, type, layers_order_list) {//передаеться jQuery обьект canvas и путь к картинке
        ...
        //минимальная и максимальная ширина при увеличении изображения
        this.min_width = 10;
                ...
                img.onload = function() {
                    ...
                    obj.image_params = {
                        ...
                        dragstart: function(layer) {
                            ...
                            //прячем контрол панель
                            obj.control_panel('hide');
                        },
        ...
        switch (action) {
            case 'show': {
                ...
                if(obj.src.indexOf('.svg') != -1) $.post(ajax_requests_path, {
                    ...
                    panel.find('.svg-color-select').find('.svg-preview').html(data);
                });//bottom_mob_area
                else panel.find('.svg-color-select').find('.svg-preview').html('Данная ф-ция доступна только для svg файлов');
    ...
    scale() {//ресайзит элементы при смене масштаба
        let obj = this,
...
class Canvas {
    ...
    print_canvas() {
        var dataUrl = this.getPng(); //attempt to save base64 string to server using this var
        /*dataUrl = 'data:image/png;base64,' + escape(dataUrl.replace('data:image/png;base64,', ''));
        console.log(dataUrl);
        window.open(dataUrl);*/
        /*var image = new Image();
        image.src = dataUrl;
        ...
        popupWin.document.write('<html><head><link rel="stylesheet" type="text/css" href="style.css" /></head><body onload="window.print()"><img src="'+dataUrl+'"></html>');
