...
$(document).ready(function($) {
    ...
    $('form[name="append-text"]').submit(function() {
        let text = $(this).find('textarea').val(),
            settings = {
                font_family: $(this).find('textarea').css('font-family')
            },
            form = $(this);
        let test_text = text.replace(/\n/gi, '<br>');
        test_text = $('<div>'+test_text+'</div>').css({
        ...
        $('body').append(test_text);
        let viewBox = '0 0 ' + test_text.width() + ' ' + test_text.height();
        /**/
        test_text.remove();
        $.post(ajax_requests_path, {
            request: 'get_text_svg_path',
            text: text,
            font_family: settings.font_family,
            font_size: '16px',
            viewBox: viewBox
        }, function (data) {
            main_canvas.addImage(data);
        });
        return false;
    });
...
class layers_group {//класс отвечающий за группы изображений
    constructor(canvas, src, type, settings) {//передаеться jQuery обьект canvas и путь к картинке
...
class Canvas {
    ...
    addImage(src) {
        this.layer_groups.push(new layers_group(this.canvas, src, 'image'));
    }
