...
$(document).ready(function() {
    ...
    /**/
    $('[name="my_file"]').change(function() {
        let $input = $(this),
            fd = new FormData,
            loading = $(this).parents('ul').find('.loading');
        loading.addClass('active');
        fd.append('img', $input.prop('files')[0]);

        $.ajax({
            url: ajax_requests_path,
            data: fd,
            processData: false,
            contentType: false,
            type: 'POST',
            success: function (data) {
                ...
                $input.parents('ul').find('.icons').append('<a href="'+data+'" class="icon-point" style="background-image: url('+data+');"></a>');
                //$('#user-photo-preview').attr('src', data);
            }
        });
    });
    ...
    $('#slider1').slider({
        ...
        slide: function( event, ui ) {
            $('.polotno-count').html(ui.value);
            $('[name="polotno_count"]').val(ui.value);
        }
    });
    $('input[name="polotno_count"]').change(function() {
        $('#slider1').slider('option', 'value', $(this).val());
        $('.polotno-count').html($(this).val());
    });

    $(document).click(function() {
        $('header .top-nav-bar .clicked.active').removeClass('active');
    });
    $('header .top-nav-bar .clicked').click(function (e) {
        e.stopPropagation();
        $(this).siblings('.active').removeClass('active');
        $(this).toggleClass('active');
    });
    $('header .top-nav-bar .sub').click(function (event) {
